...
#                              "Master_mpagominimo" )   #aqui van las columnas que se quieren eliminar
...
write_yaml(  palancas,  paste0( "~/buckets/b1/work/palanca_",  palancas$version  ,".yaml" ) )
...
AgregarVariables  <- function( dataset )
{
  ...
  dataset[ , mv_ctransacc_tarj     := rowSums( cbind(ctarjeta_visa_transacciones, ctarjeta_master_transacciones) , na.rm=TRUE ) ]
  
  dataset[ , mv_minvers_pesos     := rowSums( cbind(minversion1_pesos, minversion2) , na.rm=TRUE ) ]
  
  dataset[ , mv_liquidez          := rowSums( cbind(mcuentas_saldo,mv_minvers_pesos) , na.rm=TRUE ) ]
  
  dataset[ , mv_a_deuda           := rowSums(cbind( mprestamos_personales, mprestamos_prendarios, mprestamos_hipotecarios) , na.rm=TRUE ) ]
...
# dataset[ , mv_mlimitecompra     := rowSums( cbind( Master_mlimitecompra,  Visa_mlimitecompra) , na.rm=TRUE ) ]
  ...
  #Ratios
  dataset[ , mvr_liq_deuda        := mv_liquidez / ifelse(is.na(mv_a_deuda) | mv_a_deuda==0,1,mv_a_deuda) ]

  dataset[ , mvr_msaldototal      := mv_msaldototal / mv_mlimitecompra ]
  
  #Interacciones
  dataset[ , int_vol_payrol       := cpayroll_trx * mpayroll]
  
  #Agregaciones
  dataset[ , ag_servicios         := rowSums( cbind(cseguro_vida, cseguro_auto, cseguro_vivienda,
                                                    cseguro_accidentes_personales, ccaja_seguridad, cprestamos_hipotecarios,
                                                    cprestamos_prendarios, cprestamos_personales, tcuentas) , na.rm=TRUE ) ]
  
  ##nuevas
  
  dataset[ , int_rent_ac_est      := ifelse(cliente_antiguedad>12, mrentabilidad_annual * cliente_antiguedad/12,
...
correr_todo  <- function( palancas )
{
  ...
  dataset1  <- fread( "paquete_premium_202009.csv")
  dataset2  <- fread( "paquete_premium_202011.csv")
  ...
  #Grabo el dataset
  fwrite( dataset,
          paste0( "dataset_epic_simple_", palancas$version, ".csv.gz" ),
          logical01 = TRUE,
          sep= "," )
